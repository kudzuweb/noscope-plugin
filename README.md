...
a run that died, and a session sitting there doing nothing is neither. Nothing installs it and it
is not always on. Opening an incident starts it if it is not already running, and the last
incident to end stops it — the last, not any, so one run finishing never leaves another unwatched.
Every ten minutes it looks at the runs and seats that registered themselves — a run registers when
it opens, a seat when it joins — and reports any that has taken no turn for too long and is
