...
//
//   node incident_daemon.mjs install [--minutes N]   write and load the agent (default every 10)
//   node incident_daemon.mjs status                  whether it is loaded, and what it last found
//   node incident_daemon.mjs uninstall               unload and remove it
//
...
import { NOSCOPE_HOME } from "./incident_lib.mjs";
...
if (!["install", "status", "uninstall"].includes(mode)) {
  console.error("usage: incident_daemon.mjs install [--minutes N] | status | uninstall");
  process.exit(2);
}

// The watcher runs anywhere; only the timer is macOS's. Saying so beats installing nothing silently.
if (platform() !== "darwin") {
  console.log(`this installs a launchd agent and this machine is ${platform()}.`);
...
console.log(`${LABEL} installed: ${SHIM}, every ${minutes} minute(s)`);
