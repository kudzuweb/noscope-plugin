...
if (kind === "sizeup") {
  ...
  out = { incident: state, checklist, rules: checklistText(), sinceLastPlan, returns: fieldListOf("ActionPlan"), models: { leader: resolveModel("leader", cfg) ?? "claude-sonnet-5", provider: "claude-code" }, seatModel: resolveModel("planner", cfg) ?? "claude-opus-5", ask: "Draft this operational period's tactics as an ActionPlan, a suggestion for the Incident Commander: units to open or close, tasks with resource, inputs, expected output, completion criteria, dependencies and the evidence each reads, the smallest model that fits each (Opus only with a modelWhy), what each task settles among situation.open, and any reassignment taken. Independent work runs in the same period." };
} else if (kind === "orientation") {
  const u = state.units.find((x) => x.id === id); if (!u) { console.error(`no unit ${id}`); process.exit(1); }
  const siblings = state.units.filter((x) => x.parentId === u.parentId && x.id !== u.id && x.status === "active").map((x) => ({ id: x.id, objective: x.objective }));
  const children = state.units.filter((x) => x.parentId === u.id && x.status === "active").map((x) => ({ id: x.id, objective: x.objective }));
  out = { objective: state.incident.objective, periodObjectives: state.period?.objectives ?? [], unit: { id: u.id, objective: u.objective, equipment: u.equipment, bashAllowlist: u.bashAllowlist }, hierarchy: { parent: u.parentId, siblings, children } };
