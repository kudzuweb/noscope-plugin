...
|---|---|
| A worker saying it is waiting on a file another worker holds (`… is waiting on src/x.ts (held by 001-t02)`) | Decide, and say so in one line. It has already taken every file that was free and is getting on with those, so the question is only what to do about the rest. The usual answer is to let it wait, and it will be told when the file frees. The others are to reorder the work, to tell the holder to release what it is done with (`node S/incident_apply.mjs release $F/incident.json <holder> <path>`), or to narrow the waiting task's scope so it no longer needs the file. Never tell it to write the file anyway; two workers writing one file is the merge conflict the reservation exists to prevent. A conflict between your unit and another is not yours: send it up to the IC, because you cannot see the other unit's work. |
| `DEADLOCK` — the waiting runs in a circle | No release is coming, so waiting cannot resolve it. Break it yourself: pick one worker in the circle, have it release a file, and reorder its work behind the other. Tell the IC what you did in one line. A circle that crosses units is the IC's to break. |
| `[noscope] applied by hook` naming a stalled seat (`… has done nothing for 40 minutes and is waiting on nothing`, or `… joined 40 minutes ago and has never taken a turn`) | The failsafe found a worker of yours that stopped. Read the why it quotes before doing anything: a launcher error or an API error is the seat never having run, so respawn the task; its own last words saying it is waiting on something that is not running means the work died under it, so end the task and assign it again. Do not message a finished subagent to ask; that costs a turn and stops again. |
