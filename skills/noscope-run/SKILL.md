...
| 5. Apply | `node S/incident_apply.mjs plan $F/incident.json $F/draft-<period>.json`. For each new unit: `bash S/launch-session.sh leader $F <the unit's leader model id> `$(node S/incident_name.mjs $F leader <unit-id>)` <unit-id>`. A tab opens running a session under that name whose first prompt is `/noscope-lead $F <unit-id>`; it orients itself from the record and sends you `<name> is up for unit <unit-id>`. Wait for that message before sending the unit anything. The leader session stays for the unit's life and across your own handoffs; every call to it is a `SendMessage` to that name. The launcher waits for the session to come up and exits non-zero if it never does, and prints what it tried, why the tab stopped if it got far enough to say, the command a human could run, and that the decision is yours. Read the reason first: an overloaded or rate-limited provider is worth waiting out and launching again, a model the catalogue does not carry is a wrong id you can correct, an authentication failure is the human's and is a question rather than a retry, and a tab that reported nothing never ran claude at all. Make it: in an attended run give the human the command and go on without the seat; in an unattended run close the unit or reassign its slice at your next turn, and say which and why. Either way do not wait on a check-in that will not arrive. Where Warp is absent it prints the same command. |
...
| Item | Detail |
|---|---|
| A stalled seat, reported by the failsafe | The watcher runs on a timer outside every session, because nothing inside a run can see a seat that has simply stopped: the loop hook pushes a session that is working and the heartbeat restarts a run that died, and a session sitting there is neither. What it finds reaches you as `[noscope] applied by hook`, naming the seat, how long it has done nothing, and why — quoted from the error that stopped it or from its own last words. It only ever reports a seat that is waiting on nothing, so there is always something to do. Read the why first, as you do a failed launch: a launcher or authentication error is the seat never having run, and you relaunch it or hand the command over; its own last words saying it waits on work that is not running mean the work died under it, so reassign the unit's slice or close the unit. A leader's stall is yours; a task's is its leader's, so tell the leader rather than reaching past it. Never wait on a seat the watcher has reported. |
| Two units wanting one file | A writing task takes every file that is free and waits for the rest, so a conflict never stops work that can go on. A conflict inside one unit is its leader's. One across units reaches you, because neither leader can see the other's work: decide which unit has the file, say so to both in one line each, and if the answer is that the other must wait, say that — it is told when the file frees. A `DEADLOCK` line means the waiting runs in a circle and no release is coming, so name the unit that gives way and tell it to release. Two units writing one file is a decomposition that did not finish, so say in your next situation whether the scopes need redrawing. |
