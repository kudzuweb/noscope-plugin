...
node -e 'const fs=require("fs");const p=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));p.createUnits=[{ref:"x",parent:"001-command",type:"base",objective:"something",leader:{provider:"claude-code",model:"claude-sonnet-5"},equipment:[],bashAllowlist:[]}];p.createTasks=[];fs.writeFileSync(process.argv[2],JSON.stringify(p))' "$F/draft-1.json" "$T/noscope-unit.json" 2>/dev/null || true
...
{"createUnits":[{"ref":"big","parent":"001-command","type":"base","objective":"Work that is hard to decompose","scope":"the whole of the undecomposed work","leader":{"provider":"claude-code","model":"claude-fable-5-1"},"equipment":["Read"],"bashAllowlist":[]}],
